...
contract Reward {
    ...
    function addEpoch(
        ...
        uint endTime,
        uint totalReward
    ) external onlyAdmin returns (uint, uint) {
        ...
        (uint epochId, uint accurateTotalReward) = _addEpoch(
            startTime,
            endTime,
            totalReward
        );
    ...
    function _addEpoch(
        ...
        uint rewardPerSecond = (totalReward * RewardMultiplier) /
            (endTime - startTime);
        uint epochId = epochInfo.length;
        epochInfo.push(EpochInfo(startTime, endTime, rewardPerSecond, 1, 1));
        uint accurateTotalReward = ((endTime - startTime) * rewardPerSecond) /
            RewardMultiplier;
        return (epochId, accurateTotalReward);
    }
    ...
    function _pendingRewardSingle(
        ...
        uint last = lastClaimTime >= epoch.startTime
            ? lastClaimTime
            : epoch.startTime;
        ...
        uint reward = (epoch.rewardPerSecond * (end - last) * power) /
    ...
    ) public returns (uint[] memory rewards) {
    ...
    function claimReward(
        ...
        for (uint i = startEpoch; i <= endEpoch; i++) {
            ...
            checkEpoch(i);
            (uint reward_i, bool finished) = _pendingRewardSingle(
                tokenId,
    ...
    function getEpochIdByTime(uint _time) public view returns (uint) {
    ...
    function getEpochInfo(uint epochId) public view returns (uint, uint, uint) {
        ...
        uint totalReward = ((epoch.endTime - epoch.startTime) *
    ...
    function getPendingRewardSingle(
        uint tokenId,
        uint epochId
    ) public view returns (uint reward, bool finished) {
